...
void main(void) {
    ...
    vec4 emiss = texture2D(texEmiss, f_uv);
    vec4 c = vec4(1.0);
    float idx = sampled.r;
    if (tex_mode == TEX_MODE_16BPP || tex_mode == TEX_MODE_BILINEAR_16BPP) {
        c = sampled.bgra;
    } else if (tex_mode == TEX_MODE_WORLDPAL || tex_mode == TEX_MODE_BILINEAR) {
        if (idx == 0.0 && (blend_mode == D3DBLEND_SRCALPHA || blend_mode == D3DBLEND_INVSRCALPHA)) discard;
        c = texture2D(worldPalette, vec2(idx, 0.5));
        float litidx = texture2D(worldPaletteLights, vec2(idx, clamp(f_light,0.0,1.0)/LIGHT_DIVISOR)).r;
        c.rgb += texture2D(worldPalette, vec2(litidx,0.5)).rgb * light_mult;
    }
    c *= f_color * albedoFactor;
    c.rgb += emiss.rgb * emissiveFactor * 0.1 + colorEffects_add;
    c.rgb *= colorEffects_filter * colorEffects_fade;
    if (c.a < 0.01) discard;
