...
int stdSound_BufferUnlock(stdSound_buffer_t* sound, void* buffer, int bufferRead)
{
    ...
    {
        // Pyra: convert packed signed 24-bit PCM to 16-bit in place.  The old
        // path allocated a second full-size buffer, memcpy'd the sample data,
        // memset the destination, converted it, then freed the temporary block.
        // Walking backwards is overlap-safe because 3 input bytes become 2.
        uint8_t* pcm = (uint8_t*)sound->data;
        const size_t sampleCount = (size_t)sound->bufferBytes / 3;
        for (size_t i = sampleCount; i-- > 0; )
        {
            const size_t src = i * 3;
            const size_t dst = i * 2;
            int32_t sample24 = (int32_t)((uint32_t)pcm[src] |
                                         ((uint32_t)pcm[src + 1] << 8) |
                                         ((uint32_t)pcm[src + 2] << 16));
            if (sample24 & 0x00800000)
                sample24 |= (int32_t)0xFF000000;
            const int16_t sample16 = (int16_t)(sample24 >> 8);
            pcm[dst] = (uint8_t)(sample16 & 0xFF);
            pcm[dst + 1] = (uint8_t)(((uint16_t)sample16 >> 8) & 0xFF);
        }

        sound->bufferBytes = (int)(sampleCount * 2);
        sound->format = sound->bStereo ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;
    }
