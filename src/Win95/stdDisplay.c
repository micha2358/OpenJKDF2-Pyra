...
int stdDisplay_SetMode(unsigned int modeIdx, const void *palette, int paged)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, newW, newH, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, Video_menuBuffer.sdlSurface->pixels);
    
    glGenTextures(1, &Video_overlayTexId);
    glBindTexture(GL_TEXTURE_2D, Video_overlayTexId);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, newW, newH, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, Video_overlayMapBuffer.sdlSurface->pixels);
