...
int rdClip_SphereInFrustum(const rdClipFrustum* NO_ALIAS frust, const rdVector3* NO_ALIAS pos, flex_t rad)
{
    ...
    // projectType cannot change during one frustum test. Cache it once instead
    // of repeatedly dereferencing the current camera for every plane.
    const BOOL bPerspective = (rdCamera_pCurCamera->projectType == rdCameraProjectType_Perspective);
