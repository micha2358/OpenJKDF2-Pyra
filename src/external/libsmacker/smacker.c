...
static char smk_render_palette(struct smk_video_t* s, uint8_t* p, uint32_t size)
{
	...
	/*
	 * Defensive guard for corrupt/underflowed chunk sizes.
	 * 0xFFFFFFFC is the classic result of unsigned (0 - 4).
	 */
	if (size > 0x10000000U)
	{
		fprintf(stderr,
			"libsmacker::smk_render_audio() - ERROR: invalid input size %u.\n",
			size);
		return -1;
	}
...
static char smk_render(smk s)
{
	...
	for (track = 0; track < 7; track ++)
	{
		if (s->frame_type[s->cur_frame] & (0x02 << track))
		{
			...
			/*
			 * Validate the complete audio record before subtracting
			 * the 4-byte size header. Otherwise size == 0 underflows
			 * to 0xFFFFFFFC when passed as size - 4.
			 */
			if (size < 4 || size > i)
			{
				fprintf(stderr,
					"libsmacker::smk_render() - ERROR: frame %lu: "
					"invalid audio[%u] record size %u (remaining %u bytes).\n",
					s->cur_frame, track, size, i);
			...
			{
				if (smk_render_audio(&s->audio[track], p + 4, size - 4) < 0)
				{
					fprintf(stderr,
						"libsmacker::smk_render() - ERROR: audio[%u] decode failed.\n",
						track);
					goto error;
				}
			}
