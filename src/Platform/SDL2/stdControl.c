 ...
 * Pyra rumble.
 *
 * Keep haptics completely separate from the gameplay joystick.
 * SDL enumerates the TWL6040 vibrator through SDL_INIT_HAPTIC
 * while the normal joystick/controller stays exclusively in the input path.
 */
static SDL_Haptic *stdControl_pPyraHaptic = NULL;

static void stdControl_PyraHapticInit(void)
{
    if (stdControl_pPyraHaptic)
        return;

    if (SDL_InitSubSystem(SDL_INIT_HAPTIC) != 0)
    {
        stdPlatform_Printf("Pyra haptic init failed: %s\n", SDL_GetError());
        return;
    }
    ...
    if (count <= 0)
    {
        stdPlatform_Printf("Pyra haptic: no SDL haptic devices found\n");
        return;
    }
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        stdPlatform_Printf("SDL haptic %d: %s\n",
                           i, name ? name : "(unknown)");

        if (name &&
            (strstr(name, "twl6040") != NULL ||
             strstr(name, "vibrator") != NULL))
        {
            preferred = i;
            break;
        }
    ...
    stdControl_pPyraHaptic = SDL_HapticOpen(preferred);
    if (!stdControl_pPyraHaptic)
    {
        stdPlatform_Printf("Pyra haptic open failed: %s\n", SDL_GetError());
        return;
    }

    if (!SDL_HapticRumbleSupported(stdControl_pPyraHaptic))
    {
        stdPlatform_Printf("Pyra haptic device has no rumble support\n");
        SDL_HapticClose(stdControl_pPyraHaptic);
        stdControl_pPyraHaptic = NULL;
        return;
    }

    if (SDL_HapticRumbleInit(stdControl_pPyraHaptic) != 0)
    {
        stdPlatform_Printf("Pyra haptic rumble init failed: %s\n",
                           SDL_GetError());
        SDL_HapticClose(stdControl_pPyraHaptic);
        stdControl_pPyraHaptic = NULL;
        return;
    ...
    stdPlatform_Printf("Pyra haptic rumble enabled: %s\n",
                       SDL_HapticName(preferred)
                           ? SDL_HapticName(preferred)
                           : "(unknown)");
}

static void stdControl_PyraHapticShutdown(void)
{
    if (stdControl_pPyraHaptic)
    {
        SDL_HapticRumbleStop(stdControl_pPyraHaptic);
        SDL_HapticClose(stdControl_pPyraHaptic);
        stdControl_pPyraHaptic = NULL;
    }
}

void stdControl_PyraRumbleHit(void)
{
    if (!stdControl_pPyraHaptic)
        return;

    /* Rumble: 100% strength for a 120 ms pulse. */
    if (SDL_HapticRumblePlay(stdControl_pPyraHaptic, 1.0f, 120) != 0)
    {
        static int reported = 0;
        if (!reported)
        {
            stdPlatform_Printf("Pyra haptic rumble failed: %s\n",
                               SDL_GetError());
            reported = 1;
        }
    }
...
void stdControl_ToggleCursor(int a)
{
    ...
    SDL_bool relativeMode = (SDL_bool)!!a;
    if (SDL_GetRelativeMouseMode() != relativeMode)
