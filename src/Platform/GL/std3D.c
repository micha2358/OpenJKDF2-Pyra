...
#define PYRA_STREAM_BUFFER_COUNT 3
static GLuint world_vbo_all[PYRA_STREAM_BUFFER_COUNT];
static GLuint world_ibo_triangle[PYRA_STREAM_BUFFER_COUNT];
static unsigned int world_stream_idx = 0;

D3DVERTEX* menu_data_all = NULL;
GLushort* menu_data_elements = NULL;
static GLuint menu_vbo_all[PYRA_STREAM_BUFFER_COUNT];
static GLuint menu_ibo_triangle[PYRA_STREAM_BUFFER_COUNT];
static unsigned int menu_vbo_stream_idx = 0;
static unsigned int menu_ibo_stream_idx = 0;

/*
 * Pyra / SGX544 streaming buffers.  Dynamic geometry is written into a
 * rotating set of VBOs/IBOs instead of overwriting the buffer most recently
 * submitted to the GPU.  GLES2 has no mandatory MapBuffer/fence API, so a
 * small multi-buffer ring is the portable way to give the driver several
 * submissions of breathing room without an explicit CPU/GPU synchronization.
 */
static GLuint std3D_PyraNextMenuVbo(void)
{
    menu_vbo_stream_idx = (menu_vbo_stream_idx + 1) % PYRA_STREAM_BUFFER_COUNT;
    return menu_vbo_all[menu_vbo_stream_idx];
}

static GLuint std3D_PyraNextMenuIbo(void)
{
    menu_ibo_stream_idx = (menu_ibo_stream_idx + 1) % PYRA_STREAM_BUFFER_COUNT;
    return menu_ibo_triangle[menu_ibo_stream_idx];
}
...
void std3D_generateIntermediateFbo(int32_t width, int32_t height, std3DIntermediateFbo* pFbo, int isFloat)
{
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, pFbo->rbo);
...
void std3D_generateFramebuffer(int32_t width, int32_t height, std3DFramebuffer* pFb)
{
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, pFb->rbo);
    if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        stdPlatform_Printf("std3D: ERROR, Framebuffer is incomplete!\n");
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
int init_resources()
{
    ...
    glGenBuffers(PYRA_STREAM_BUFFER_COUNT, world_vbo_all);
    glGenBuffers(PYRA_STREAM_BUFFER_COUNT, world_ibo_triangle);

    glGenBuffers(PYRA_STREAM_BUFFER_COUNT, menu_vbo_all);
    glGenBuffers(PYRA_STREAM_BUFFER_COUNT, menu_ibo_triangle);
    world_stream_idx = 0;
    menu_vbo_stream_idx = 0;
    menu_ibo_stream_idx = 0;
...
void std3D_FreeResources()
{
    ...
    glDeleteBuffers(PYRA_STREAM_BUFFER_COUNT, world_vbo_all);
    glDeleteBuffers(PYRA_STREAM_BUFFER_COUNT, world_ibo_triangle);

    glDeleteBuffers(PYRA_STREAM_BUFFER_COUNT, menu_vbo_all);
    glDeleteBuffers(PYRA_STREAM_BUFFER_COUNT, menu_ibo_triangle);
...
int std3D_StartScene()
{
    ...
    // Rotate once per rendered scene.  The same slot is used for the scene's
    // vertex/index uploads, while the previous two slots can remain in flight.
    world_stream_idx = (world_stream_idx + 1) % PYRA_STREAM_BUFFER_COUNT;

    // Describe our vertices array to OpenGL (it can't guess its format automatically).
    // No dummy glBufferData is needed here; the real store is supplied when
    // the completed render list is uploaded.
    glBindBuffer(GL_ARRAY_BUFFER, world_vbo_all[world_stream_idx]);
...
void std3D_DrawMenu()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, std3D_PyraNextMenuVbo());
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, std3D_PyraNextMenuIbo());
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, GL_tmpTrisAmt * 3 * sizeof(GLushort), menu_data_elements, GL_STREAM_DRAW);

    // The index count is already known. Querying GL_BUFFER_SIZE here forces a
    // synchronous driver round-trip on some GLES2 drivers (notably costly on SGX).
...
void std3D_DrawSimpleTex(std3DSimpleTexStage* pStage, std3DIntermediateFbo* pFbo, GLuint texId, GLuint texId2, GLuint texId3, flex_t param1, flex_t param2, flex_t param3, int gen_mips)
{
    ...
    *(uint32_t*)&GL_tmpVertices[3].nz = 0;
    ...
    // Attribute arrays are already enabled above; do not submit the same GLES
    // state changes a second time.
    rdDDrawSurface* last_tex = (rdDDrawSurface*)(intptr_t)-1;
    int last_tex_idx = 0;
    //GLushort* data_elements = malloc(sizeof(GLushort) * 3 * GL_tmpTrisAmt);
    for (int j = 0; j < GL_tmpTrisAmt; j++)
...
/* Pyra SGX hot-path state cache.  This cache is deliberately scoped to the
 * world render-list pass; it is invalidated at the beginning of every pass,
 * so GL work performed by menus/post-processing cannot desynchronise it. */
static GLint pyra_hot_active_tex = -1;
static GLuint pyra_hot_bound_tex[5] = {~0u, ~0u, ~0u, ~0u, ~0u};
static GLenum pyra_hot_blend_src = (GLenum)~0u;
static GLenum pyra_hot_blend_dst = (GLenum)~0u;
static GLenum pyra_hot_depth_func = (GLenum)~0u;
static GLboolean pyra_hot_depth_mask = 2;
static GLenum pyra_hot_cull_face = (GLenum)~0u;
static GLint pyra_hot_tex_mode = -0x7fffffff;
static GLint pyra_hot_blend_mode = -0x7fffffff;
static rdDDrawSurface* pyra_hot_material = (rdDDrawSurface*)(intptr_t)-1;
static int pyra_hot_material_filter = -1;
static int pyra_hot_material_bloom = -1;

static void pyra_hot_reset(void)
{
    pyra_hot_active_tex = -1;
    for (int i = 0; i < 5; ++i) pyra_hot_bound_tex[i] = ~0u;
    pyra_hot_blend_src = (GLenum)~0u;
    pyra_hot_blend_dst = (GLenum)~0u;
    pyra_hot_depth_func = (GLenum)~0u;
    pyra_hot_depth_mask = 2;
    pyra_hot_cull_face = (GLenum)~0u;
    pyra_hot_tex_mode = -0x7fffffff;
    pyra_hot_blend_mode = -0x7fffffff;
    pyra_hot_material = (rdDDrawSurface*)(intptr_t)-1;
    pyra_hot_material_filter = -1;
    pyra_hot_material_bloom = -1;
}

static void pyra_hot_texture(GLint unit, GLuint tex)
{
    if (pyra_hot_active_tex != unit) {
        glActiveTexture(GL_TEXTURE0 + unit);
        pyra_hot_active_tex = unit;
    }
    if (unit >= 0 && unit < 5 && pyra_hot_bound_tex[unit] != tex) {
        glBindTexture(GL_TEXTURE_2D, tex);
        pyra_hot_bound_tex[unit] = tex;
    }
}

static void pyra_hot_blend(GLenum src, GLenum dst)
{
    if (pyra_hot_blend_src != src || pyra_hot_blend_dst != dst) {
        glBlendFunc(src, dst);
        pyra_hot_blend_src = src;
        pyra_hot_blend_dst = dst;
    }
}

static void pyra_hot_depthfunc(GLenum func)
{
    if (pyra_hot_depth_func != func) { glDepthFunc(func); pyra_hot_depth_func = func; }
}

static void pyra_hot_depthmask(GLboolean enabled)
{
    if (pyra_hot_depth_mask != enabled) { glDepthMask(enabled); pyra_hot_depth_mask = enabled; }
}

static void pyra_hot_cullface(GLenum face)
{
    if (pyra_hot_cull_face != face) { glCullFace(face); pyra_hot_cull_face = face; }
}

static void pyra_hot_texmode(GLint mode)
{
    if (pyra_hot_tex_mode != mode) { glUniform1i(uniform_tex_mode, mode); pyra_hot_tex_mode = mode; }
}

static void pyra_hot_blendmode(GLint mode)
{
    if (pyra_hot_blend_mode != mode) { glUniform1i(uniform_blend_mode, mode); pyra_hot_blend_mode = mode; }
}

void std3D_DoTex(rdDDrawSurface* tex, rdTri* tri, int tris_left)
{
    if (!tex) {
        pyra_hot_texture(3, blank_tex); // emissive
        pyra_hot_texture(4, blank_tex); // displace

        pyra_hot_texture(0, blank_tex_white);
        pyra_hot_texmode(TEX_MODE_TEST);
        pyra_hot_blendmode(2);
        pyra_hot_material = NULL;
        return;
    }

    /* Material parameters are constant for a texture.  Avoid resending the
     * same uniform block when a material reappears in adjacent flag batches. */
    int pyra_material_changed = (pyra_hot_material != tex ||
                                 pyra_hot_material_filter != jkPlayer_enableTextureFilter ||
                                 pyra_hot_material_bloom != jkPlayer_enableBloom);
    int tex_id = tex->texture_id;
    if (pyra_hot_active_tex != 0) { glActiveTexture(GL_TEXTURE0); pyra_hot_active_tex = 0; }
    if (tex_id == 0)
        pyra_hot_texture(pyra_hot_active_tex, blank_tex_white);
    else
        pyra_hot_texture(pyra_hot_active_tex, tex_id);

    int emiss_tex_id = tex->emissive_texture_id;
    if (pyra_hot_active_tex != 3) { glActiveTexture(GL_TEXTURE3); pyra_hot_active_tex = 3; }
    if (emiss_tex_id == 0) {
        pyra_hot_texture(pyra_hot_active_tex, blank_tex);
    }
    else {
        //printf("emissive tex id %x\n", emiss_tex_id);
        pyra_hot_texture(pyra_hot_active_tex, emiss_tex_id);

        // HACK
        if (tri[0].flags & 0x600) {
            //pyra_hot_blendmode(6);
            //last_flags |= 0x200;
        }

        
        /* The old code scanned all following triangles here, but its only
         * operation had been commented out.  On Cortex-A15 that was pure
         * per-batch CPU work, so the no-op scan is removed. */
    }

    int displace_tex_id = tex->displacement_texture_id;
    if (pyra_hot_active_tex != 4) { glActiveTexture(GL_TEXTURE4); pyra_hot_active_tex = 4; }
    if (displace_tex_id == 0) {
        pyra_hot_texture(pyra_hot_active_tex, blank_tex);
    }
    else {
        pyra_hot_texture(pyra_hot_active_tex, displace_tex_id);
    }
    ...
    if (!jkPlayer_enableTextureFilter)
        pyra_hot_texmode(tex->is_16bit ? TEX_MODE_16BPP : TEX_MODE_WORLDPAL);
    else
        pyra_hot_texmode(tex->is_16bit ? TEX_MODE_BILINEAR_16BPP : TEX_MODE_BILINEAR);
    ...
    pyra_hot_material = tex;
    pyra_hot_material_filter = jkPlayer_enableTextureFilter;
    pyra_hot_material_bloom = jkPlayer_enableBloom;
    }

    /* Draw calls consume the albedo sampler on unit 0. */
    if (pyra_hot_active_tex != 0) { glActiveTexture(GL_TEXTURE0); pyra_hot_active_tex = 0; }
}
...
void std3D_DrawRenderList()
{
    ...
    pyra_hot_texmode(TEX_MODE_TEST);
    pyra_hot_blendmode(2);
    pyra_hot_texture(4, blank_tex);
    pyra_hot_texture(3, blank_tex);
    pyra_hot_texture(2, worldpal_lights_texture);
    pyra_hot_texture(1, worldpal_texture);
    pyra_hot_texture(0, blank_tex_white);
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, world_ibo_triangle[world_stream_idx]);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, GL_tmpTrisAmt * 3 * sizeof(GLushort), world_data_elements, GL_STREAM_DRAW);
    
    int do_batch = 0;
    
    //pyra_hot_depthfunc(GL_LESS);
    //pyra_hot_depthmask(GL_TRUE);
    //pyra_hot_cullface(GL_FRONT);
    ...
    if (last_flags & 0x800) {
        ...
        pyra_hot_depthfunc(GL_ALWAYS);
    }

    if (last_flags & 0x600) {
        
        if (last_flags & 0x200) {
            pyra_hot_blendmode(D3DBLEND_INVSRCALPHA);
            pyra_hot_blend(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
        }
        else {
            pyra_hot_blendmode(D3DBLEND_SRCALPHA);
            pyra_hot_blend(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
        }
    }
    else {
        pyra_hot_blendmode(D3DBLEND_ONE);
        pyra_hot_blend(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }
    ...
    {
        pyra_hot_depthmask(GL_FALSE);
    }

    if (last_flags & 0x10000) {
        pyra_hot_cullface(GL_BACK);
    }
    else
    {
        pyra_hot_cullface(GL_FRONT);
    ...
    // Done drawing    
    pyra_hot_texture(pyra_hot_active_tex, worldpal_texture);
    pyra_hot_cullface(GL_FRONT);
...
int std3D_AddToTextureCache(stdVBuffer *vbuf, rdDDrawSurface *texture, int is_alpha_tex, int no_alpha)
{
    ...
    if (vbuf->format.format.is16bit)
    {
        ...
        if (!is_alpha_tex)
        {
            /* The source buffer is RGB565. GLES2 supports this format natively. */
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0,
                         GL_RGB, GL_UNSIGNED_SHORT_5_6_5, image_8bpp);
            texture->pDataDepthConverted = NULL;
        }
        else
        {
            /* A1R5G5B5 is not a GLES2 packed upload type; convert to RGBA8888. */
            uint32_t row_stride = vbuf->format.width_in_bytes / 2;
            uint8_t* image_rgba = (uint8_t*)malloc(width * height * 4);
            ...
            for (uint32_t y = 0; y < height; y++)
            {
                ...
                {
                    uint16_t val = image_16bpp[(y * row_stride) + x];
                    uint8_t* dst = &image_rgba[((y * width) + x) * 4];
                    uint8_t a1 = (val >> 15) & 0x01;
                    uint8_t r5 = (val >> 10) & 0x1F;
                    uint8_t g5 = (val >> 5) & 0x1F;
                    uint8_t b5 = val & 0x1F;
                    dst[0] = (uint8_t)((r5 * 527 + 23) >> 6);
                    dst[1] = (uint8_t)((g5 * 527 + 23) >> 6);
                    dst[2] = (uint8_t)((b5 * 527 + 23) >> 6);
                    dst[3] = a1 ? 0xFF : 0x00;
                }
            }

            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0,
                         GL_RGBA, GL_UNSIGNED_BYTE, image_rgba);
            texture->pDataDepthConverted = image_rgba;
        }
